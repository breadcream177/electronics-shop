...
일반적인 REST API 서버가 아니라, PHP 페이지와 Form 요청을 중심으로 동작하는 서버 렌더링 방식의 쇼핑몰입니다.
...
이 프로젝트에서 가장 중요하게 본 부분은 "쇼핑몰처럼 상태가 계속 바뀌는 기능에서 어떤 값을 신뢰할 것인가"였습니다. 단순히 상품을 보여주고 버튼을 누르면 결제 페이지로 이동하는 흐름은 어렵지 않지만, 장바구니 금액, 주문 금액, 결제 금액, 결제 완료 상태가 서로 어긋나면 쇼핑몰 기능 전체가 신뢰하기 어려워진다고 판단했습니다.

### 1. 클라이언트 금액을 그대로 믿지 않기

처음 주문 흐름을 만들 때는 화면에 표시된 총액을 주문 생성 단계로 넘기는 방식이 가장 단순했습니다. 하지만 브라우저에서 전달되는 값은 사용자가 수정할 수 있기 때문에, 결제 금액처럼 중요한 값에는 그대로 쓰기 어렵다고 봤습니다.

그래서 주문 생성 시점에 `cart`와 `products`를 다시 조회해 서버에서 총액을 계산하도록 했습니다. 사용자가 전달한 `display_total`은 화면 표시와 비교하기 위한 값으로만 보고, 실제 주문 데이터는 DB의 상품 가격과 수량을 기준으로 만들었습니다.

이 판단 때문에 주문 로직은 조금 길어졌지만, 결제 검증 단계에서 PortOne API의 결제 금액과 DB의 `pay_amount`를 비교할 수 있는 기준이 생겼습니다.
...
### 2. 결제 완료 처리는 "결제창 응답"이 아니라 "서버 검증" 이후로 미루기

카카오페이 결제창에서 성공 응답을 받는 것만으로 주문을 완료 처리하면 구현은 간단합니다. 하지만 결제창 응답은 사용자의 브라우저를 거쳐 돌아오기 때문에, 최종 주문 상태를 바꾸는 기준으로 삼기에는 불안하다고 판단했습니다.

그래서 결제 성공 콜백 이후에도 바로 주문을 완료하지 않고, `/Payment/payment_verify.php`에서 PortOne REST API로 결제 정보를 다시 조회했습니다. 이때 `merchant_uid`, 결제 금액, 결제 상태가 DB 주문 정보와 일치할 때만 `orders.status`와 `orders.pay_status`를 `paid`로 변경했습니다.

이 방식은 단순한 "결제 연동"보다 코드가 늘어나지만, 결제 결과를 외부 API와 DB 기준으로 다시 맞춰보는 구조라서 주문 상태를 더 안전하게 관리할 수 있었습니다.
...
### 3. 새로고침과 재진입을 고려한 주문 상태 처리

결제 검증 페이지는 사용자가 새로고침하거나, 모바일 결제 후 리다이렉트 과정에서 다시 호출될 수 있습니다. 이때 이미 결제 완료된 주문에 대해 장바구니 삭제나 상태 변경이 반복되면 데이터 흐름이 꼬일 수 있다고 봤습니다.

그래서 검증 로직에서 먼저 현재 주문의 `pay_status` 또는 `status`가 이미 `paid`인지 확인했습니다. 이미 처리된 주문이면 결제 완료 후처리를 다시 실행하지 않도록 분기했고, 아직 결제 완료 전인 주문만 상태 변경과 장바구니 정리를 수행하도록 했습니다.

이 판단은 큰 기술을 붙인 해결책은 아니지만, 결제처럼 같은 요청이 여러 번 들어올 수 있는 흐름에서는 "한 번 처리된 요청을 다시 처리하지 않는 것"이 중요하다는 점을 반영한 부분입니다.

관련 파일:

- `Payment/payment_verify.php`

### 4. 미결제 주문을 새로 만들기보다 재사용/정리하기

주문서 작성 화면에서 사용자가 결제를 중단하고 다시 주문을 시도하면, 미결제 주문이 계속 쌓일 수 있습니다. 그래서 `checkout.php`에서는 최근 1시간 안에 생성된 미결제 주문이 있으면 기존 주문을 갱신하고, 오래된 미결제 주문은 정리하는 흐름을 넣었습니다.

이 방식은 주문 생성 로직을 단순 INSERT로 끝내지 않고, "사용자가 결제를 중간에 멈출 수 있다"는 실제 사용 흐름을 고려한 처리입니다. 개인 프로젝트 규모에서는 별도 배치 시스템까지 두기보다, 주문서 진입 시점에 정리하는 방식이 구현 범위에 맞다고 판단했습니다.
